...
def add_dialog(index_id=-1, title="", number="", email="", address=""):
    ...
    def alert_box(title, message):
        messagebox.showwarning(title, message, parent=add_window)
    ...
    entry_exceptions = ["Name or Number field required!", "This field required text!", "This field required number!", "Input must be number!"]
    ...
    def save_data():
        ...
        if "name" in empty_variables and "number" in empty_variables:
            alert_box("Add Contact", entry_exceptions[0])
            add_window.focus_force()
        ...
        if not "name" in empty_variables and not "number" in empty_variables:
            ...
            if not _number.isdigit():
                ...
                number_entry.insert(0, entry_exceptions[3])
                add_window.focus_force()
            else:
                field = True
...
def update_list(*args):
    ...
    if search_term == search_placeholder:
...
Label(detail_frame, image=images[4], bg="#fff").place(x=125, y=30)
...
contact_frame = Frame(root, bg="#fff", width=356, height=560, highlightbackground="#808080", highlightthickness=2)
...
search_btn = Button(contact_frame, fg="#fff", text="Search", width=10, height=1,borderwidth=0, bg="#77C2FF", activebackground="#5cb3fb", activeforeground="#fff", pady=6, font=("Arial", 12))
...
def show_contact(event=None, getindex=-1):
    ...
    if getpass:
        ...
        contact_title.place(x=30, y=145)
...
add_btn = Button(contact_frame, command=add_dialog, width=70)
update_btn = Button(contact_frame, command=update_dialog, width=70)
delete_btn = Button(contact_frame, command=delete_contact, width=70)

add_btn.place(x=5, y=507)
update_btn.place(x=122,y=507)
delete_btn.place(x=240, y=507)
